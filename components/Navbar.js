  ...
  return (
    <>
      <div className={`w-full ${showSidebar ? "sidebar-open" : ""}`} ref={navbarRef}>
        <div className={`navbar sm:h-[90px] h-[75px] border-${theme}_secondary border-b-2 w-full flex flex-grow items-center sm:p-8 py-2 px-3 justify-between bg-${theme}_secondary_high shadow-md `}>
          ...
          <>
            {showSidebar == false ? (
                ...
                <Link target="_blank" className="flex items-center"  href="https://github.com/anand346/findissues">
                    <i className={`fa fa-github text-[26px] text-${theme}_primary md:hover:text-${theme}_yellow duration-300`} ></i>
                </Link>
            ...
            <div
              ref={navbarRef}
              className={`top-0 right-0 w-[230px] sm:w-[300px] bg-${theme}_secondary_high border-${theme}_secondary border-[2px] p-3 pt-3 text-${theme}_secondary fixed h-full overflow-y-scroll z-40 md:hidden ease-in-out duration-300 ${
                showSidebar ? "translate-x-0 " : "translate-x-full"
              }`}
            >
              <div className="w-full flex flex-col items-start justify-start md:mb-5">
                <p className={`basic_search_title w-full mb-2 md:mb-4 text-${theme}_primary font-semibold italic text-[16px] lg:text-[18px]`}>
                  Languages
                </p>
                <button
                    className={`flex text-xl text-${theme}_primary md:hidden items-center cursor-pointer absolute right-5 top-2 z-50`}
                    onClick={() => setShowSidebar(!showSidebar)}
                >
                    x
                </button>
                ...
                <div className="advance_search_title w-full flex flex-col items-start justify-start md:mb-5">
                  <div className="advance_search_title mb-2 md:mb-4 flex w-full justify-start items-center">
                    <p className={`advance_search_title text-${theme}_primary font-semibold italic text-[16px] lg:text-[18px] mr-3`}>
                      Tags
                    </p>
                  </div>
                  <div
                ...
                <div className="w-full flex flex-col items-start justify-start md:mb-5">
                  ...
                  >
                        <Link href={"https://github.com/anand346/findissues#add-active-repo-"} target="_blank"  className={`first:mt-3 cursor-pointer mr-2 lang_name  px-3 py-1 text-center border-${theme}_primary border-[2px] rounded-[5px] italic font-semibold text-${theme}_primary text-[12px] lg:text-[14px] transition-all transform md:hover:scale-105 md:hover:border-dashed md:hover:text-${theme}_yellow md:hover:border-${theme}_yellow duration-300 grow`} onClick={() => setShowSidebar(false)} >
                            Add Repo?
                        </Link>        
                        <Link href={"/active-repos"} className={`${
                            isCurrentPath("/active-repos")
                                ? `bg-${theme}_secondary`
                                : ""
                            } cursor-pointer mr-2 lang_name  px-3 py-1 text-center border-${theme}_primary border-[2px] rounded-[5px] italic font-semibold text-${theme}_primary text-[12px] lg:text-[14px] transition-all transform md:hover:scale-105 md:hover:border-dashed md:hover:text-${theme}_yellow md:hover:border-${theme}_yellow duration-300 grow`} onClick={() => setShowSidebar(false)} >
                            Active Repos
                        </Link> 
                    </div>
                </div>
          ...
          <div className={`navbar_right flex space-x-5 hidden md:block `}>
            <Link target="_blank" href="https://github.com/anand346/findissues">
              <i className={`fa fa-github text-3xl text-${theme}_primary md:hover:text-${theme}_yellow duration-300`} ></i>
            </Link>
          </div>
        </div>
      </div>
