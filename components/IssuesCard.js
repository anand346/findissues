  ...
  return (
    <>
      <div
        ...
        className={`issue_card mb-3 border-2 border-${theme}_main_primary w-[100%] sm:w-[95%] rounded-sm flex flex-col justify-start items-start cursor-pointer p-3 transition-all transform md:hover:scale-105`}
      >
        <div className="title_sec w-[100%] flex justify-between items-center">
          <Link href={issue.issueUrl} className="truncate " target="_blank">
            <p id="issueLink" className={`hover:text-${theme}_main_primary truncate w-full  text-${theme}_main_secondary_low lg:text-[16px] sm:text-[14px] text-[14px] font-semibold`}>
              {/* fix text-_low according to repocard */} 
              <span className={`text-${theme}_main_primary m-0`}>
                #{issue.issueNumber}
              </span>{" "}
              {issue.issueTitle}
            </p>
        ...
        >
          <p
            className={`hover:text-${theme}_main_secondary_low truncate w-full lg:text-[16px] sm:text-[14px] text-[14px] w-full truncate text-[#a6a6c1] mb-3`}
            id="repoName"
          >
            <i
              className={`fa fa-github text-${theme}_main_primary`}
              aria-hidden="true"
              id="gitIcon"
            ></i>{" "}
            {issue.repoTitle}
        ...
        <div className="footer_sec flex lg:text-[16px] sm:text-[12px] text-[12px] flex-col sm:flex-col lg:flex-row lg:justify-between sm:justify-start lg:items-center sm:items-start  w-[100%]">
          ...
          {hydrated && (
            <p className={`issue_sec lg:text-[14px] sm:text-[12px] text-[12px] italic text-${theme}_main_yellow`}>
